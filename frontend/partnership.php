...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
  ...
  $company = filter_var(trim($_GET['company'] ?? ''), FILTER_SANITIZE_SPECIAL_CHARS);
  $name = filter_var(trim($_GET['name'] ?? ''), FILTER_SANITIZE_SPECIAL_CHARS);
  $email = filter_var(trim($_GET['email'] ?? ''), FILTER_SANITIZE_EMAIL);
  $phone = filter_var(trim($_GET['phone'] ?? ''), FILTER_SANITIZE_SPECIAL_CHARS);
  $type = filter_var(trim($_GET['type'] ?? ''), FILTER_SANITIZE_SPECIAL_CHARS);
  ...
  if ($file) {
    // If file is newly created, add header columns
    if (!$file_exists) {
      fputcsv($file, [
        "Submission Date",
  ...
  header("Location: partnership.php");
  exit();
}
?>
  ...
  <main class="partnership-container">
    ...
    <section id="partner-form-section" class="partner-form-section">
      <div class="form-glass-card">
        ...
        <form class="partner-form" action="#" method="GET" onsubmit="event.preventDefault()">
          <div class="form-row">
            <div class="form-group">
              <label for="company-name">ORGANIZATION / BRAND NAME</label>
              <input type="text" id="company-name" name="company" placeholder="Enter brand name" required />
            </div>

            <div class="form-group">
              <label for="contact-person">CONTACT PERSON</label>
              <input type="text" id="contact-person" name="name" placeholder="Enter your full name" required />
            </div>
          </div>
          ...
          <div class="form-group">
            <label for="partnership-type">PARTNERSHIP CATEGORY</label>
            <select id="partnership-type" name="type" required>
              ...
              <option value="grooming">Grooming &amp; Styling Partner</option>
              <option value="media">Media &amp; PR Partner</option>
          ...
          <div class="form-group">
            ...
            <textarea id="message" name="message" rows="4"
              placeholder="Tell us about your brand goals and partnership expectations..."></textarea>
          </div>
