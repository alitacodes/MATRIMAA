...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  try {
    ...
    $hasPassport = isset($_POST['has_passport']) ? (int) $_POST['has_passport'] : 0;
    $passportNumber = !empty($_POST['passport_number']) ? trim($_POST['passport_number']) : NULL;
    $socialLink = !empty($_POST['social_link']) ? trim($_POST['social_link']) : NULL;
    $motivation = !empty($_POST['motivation']) ? trim($_POST['motivation']) : NULL;

    if (!empty($fullName) && $email !== false && !empty($phone) && !empty($city) && $age !== false && !empty($occupation)) {
      $sql = "INSERT INTO join_us_registrations (full_name, email, phone, city, age, occupation, has_passport, passport_number, social_link, motivation) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
      ...
      mysqli_stmt_bind_param($stmt, "ssssisisss", $fullName, $email, $phone, $city, $age, $occupation, $hasPassport, $passportNumber, $socialLink, $motivation);
      ...
      header("Location: join-us.php?status=success");
      exit();
    } else {
  ...
  <main class="join-container">
    <section class="join-form-section">
      <div class="form-glass-card">
        ...
        <form class="participant-form" action="join-us.php" method="POST">
          <div class="form-row">
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input type="text" id="passport_number" name="passport_number" placeholder="Enter passport number">
            </div>
          </div>

          <div class="form-group">
            <label for="social_link">SOCIAL MEDIA LINK</label>
            <input type="url" id="social_link" name="social_link" placeholder="https://instagram.com/yourprofile">
          </div>

          <div class="form-group">
            <label for="motivation">MOTIVATION / WHY DO YOU WANT TO JOIN?</label>
            <textarea id="motivation" name="motivation" rows="4"
              placeholder="Tell us about your motivation..."></textarea>
          </div>

          <div class="form-submit-wrapper">
            <button type="submit" class="spl-btn-submit">SUBMIT APPLICATION</button>
          </div>
        </form>
      </div>
    </section>
